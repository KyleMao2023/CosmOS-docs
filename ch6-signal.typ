...
初赛版本在此做出过一个范围取舍：`epoll` 家族只提供 `epoll_create1` 占位（分配匿名 fd 以取悦 glibc 的初始化路径），`epoll_ctl` / `epoll_wait` 背后那套“兴趣集 + 边沿 / 电平 + 就绪队列”的机器并未实装——当时的判断是，二维位图注册表的 `ppoll` 已足以支撑面向的工作负载，epoll 的 $O(1)$ 就绪通知收益在当时规模下不兑现。决赛阶段的真实构建负载推翻了这个判断：cargo 与 rustc 的事件循环（mio）把管道、终端与信号统一挂进 epoll，且默认注册*边沿触发*，epoll 成为工具链能否运行的硬前提。于是我们复用本章现有的 `source_id` 与 `notify_poll_source` 通知机制，在注册表之上补全了整个家族：`epoll_ctl` 的兴趣项挂在 `EpollItem`（事件掩码、用户 `data`、双向订阅索引）上，就绪通知与 `epoll_pwait/pwait2`（含信号掩码）接入同一套代计数器防护；边沿触发以每项的就绪状态原子字实现“仅在新边沿入队一次”。这部分的机制细节见第九章主题四。
...
回顾整套设计，最核心的取舍在于用“集中式槽注册表 + 代计数器 + 键控等待队列”这一统一范式取代了传统的“每 fd 一条等待队列”。这带来了三个收益：所有阻塞点共享一条正确性经过充分验证的唤醒 / 阻塞通路；信号投递借助类型擦除句柄与任意等待队列解耦；三类并发等待（poll、futex、signal）复用同一套 ABA 防护。代价同样清晰：注册表的固定容量设定了并发等待数的上限（尽管我们用回退路径把这一限制对用户态隐藏了）；不过初赛时“集中式结构在海量连接下不及 epoll 高效”的顾虑，在 epoll 于注册表之上补全（复用 `source_id` 通知机制，见上文）后已不再是范式内部的缺口。

在这一基础上，仍有若干可改进之处。首先，实时信号（32 号以上）目前在位图层面已预留，但 `SignalNum` 枚举尚未对其建模，siginfo 的联合体负载也只保留了发送方字段，未覆盖 `SI_TIMER`、`SI_SIGIO` 等内核来源的完整语义。其次，`sigaltstack`（备用信号栈）目前只接受并校验参数而未真正投入使用，`SA_ONSTACK` 的语义因此尚未兑现。最后，`poll_source_id` 以对象地址为身份，虽然在文件生命周期内安全，但仍可考虑引入显式的代计数器或稳定 id，以在极端的地址复用场景下进一步收紧安全边界。这些都是在不动摇现有范式的前提下，可以渐进推进的方向。
