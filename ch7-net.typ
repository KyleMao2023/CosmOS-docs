...
```text
 send(frame)                            IRQ / reclaim_tx_completions()
 ──────────────                         ───────────────────────────────
 transmit_begin  → token = k            poll_transmit()（仅窥视）→ token
 tx_done[k] = false                        ├ 非阻塞槽：回收 tx_slots[k]
 wait_selected(token = k)  ◀────────────── └ 阻塞槽：tx_done[k] = true
     （以 k 为键阻塞）                                wake_selected(k)
 tx_token_ready(k) → true
 transmit_complete(k)
```
